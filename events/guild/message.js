...
*/
const config = require("../../gitignore/config.json"); //loading config file with token and prefix, and settings
const ee = require("../../gitignore/embed.json"); //Loading all embed settings like color footertext and icon ...
...
module.exports = async (client, message) => {
  try {
    ...
    //get the current prefix from the gitignore/config.json
    let prefix = config.prefix
    //the prefix can be a Mention of the Bot / The defined Prefix of the Bot
    const prefixRegex = new RegExp(`^(<@!?${client.user.id}>|${escapeRegex(prefix)})\\s*`);
    //if its not that then return and add level to user
    if (!prefixRegex.test(message.content)) return calculateLevel(message);
...
async function calculateLevel(message) {
  let mongo = require("../../lib/mongo")
  let levelScheme = require("../../src/mongoscheme/level")
  await mongo().then(mongoDB => {
    try {
      new levelScheme({
        id: message.author.id,
        guild: message.guild.id,
        $inc:{"messages":1, "chars":message.length, "points":0, "levels":0},
      }).save()
